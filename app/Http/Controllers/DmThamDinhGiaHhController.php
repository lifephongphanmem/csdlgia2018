...
class DmThamDinhGiaHhController extends Controller
{
    public function index(){
        if(Session::has('admin')){
            $model = DmThamDinhGiaHh::all();
            return view('manage.thamdinhgiahh.danhmuc.nhom.index')
                ->with('model',$model)
                ->with('pageTitle','Danh mục nhóm thẩm định giá hàng hóa');
        }else
    ...
    public function store(Request $request){
        if(Session::has('admin')){
            $inputs =  $request->all();
            $inputs['theodoi'] = 'TD';
            $model = new DmThamDinhGiaHh();
            $model->create($inputs);
            return redirect('dmthamdinhgiahh');
        }else
            return view('errors.notlogin');
    }
    ...
    public function show(Request $request){
        ...
        $inputs = $request->all();
        $id = $inputs['id'];
        $model = DmThamDinhGiaHh::findOrFail($id);
        $check = 0;
        $check1 = 0;
