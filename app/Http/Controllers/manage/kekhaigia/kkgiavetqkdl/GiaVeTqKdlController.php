...
class GiaVeTqKdlController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = Company::join('companylvcc','companylvcc.maxa','=','company.maxa')
                    ...
                    ->where('companylvcc.manghe','TQKDL')
                ...
                $ttql = District::where('mahuyen',$modeldmnghe->mahuyen)
                    ->first();
                return view('manage.kkgia.vetqkdl.kkgia.kkgiadv.ttdn')
                    ->with('model', $model)
                    ->with('modeldv',$modeldv)
                    ->with('inputs',$inputs)
                    ->with('ttql',$ttql)
                    ->with('pageTitle', 'Danh sách doanh nghiệp kê khai giá vé tham quan khu du lịch');
    ...
    public function index(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                $model = GiaVeTqKdl::where('maxa', $inputs['masothue'])
                ...
                if(isset($modeldn)) {
                    $modeldv = Town::where('maxa', $modeldn->mahuyen)
                        ->first();
                    return view('manage.kkgia.vetqkdl.kkgia.kkgiadv.index')
                        ->with('model', $model)
                        ->with('modeldn', $modeldn)
                        ->with('modeldv', $modeldv)
                        ->with('inputs', $inputs)
                        ->with('pageTitle', 'Danh sách hồ sơ kê khai giá vé tham quan khu du lịch');
                }else
                    return view('errors.perm');
    ...
    public function create(Request $request){
        if (Session::has('admin')) {
            ...
            $modellk = '';
            if(isset($modeldn)) {
                $delctdf = GiaVeTqKdlCt::where('trangthai','CXD')
                    ->where('maxa',$inputs['masothue'])->delete();
                $idlk = GiaVeTqKdl::where('maxa',$inputs['masothue'])
                    ->where('trangthai','DD')
                    ->max('id');
                if(isset($idlk)){
                    $modellk = GiaVeTqKdl::where('id',$idlk)
                        ->first();
                    $modellkct = GiaVeTqKdlCt::where('mahs',$modellk->mahs)
                        ->get();
                    $inputs['socvlk'] = $modellk->socv;
                    $inputs['ngaycvlk'] = $modellk->ngaynhap;
                    foreach($modellkct as  $ctdf){
                        $addct = new GiaVeTqKdlCt();
                ...
                $modelct = GiaVeTqKdlCt::where('mahs',$inputs['mahs'])
                    ->get();

                return view('manage.kkgia.vetqkdl.kkgia.kkgiadv.create')
                    ->with('modeldn', $modeldn)
                    ->with('modelct',$modelct)
                    ->with('modellk',$modellk)
                    ->with('inputs', $inputs)
                    ->with('pageTitle', 'Kê khai giá vé tham quan khu du lịch thêm mới');
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->create($inputs)){
                    $modelctdf = GiaVeTqKdlCt::where('mahs',$inputs['mahs'])
                        ->update(['trangthai' => 'XD']);
                }
                return redirect('kekhaigiavetqkdl?&masothue='.$inputs['maxa']);
            } else {
                return view('errors.perm');
            }
        }else
    ...
    public function show(Request $request){
        if (Session::has('admin')) {
            $input = $request->all();
            $mahs = $input['mahs'];
            $modelkk = GiaVeTqKdl::where('mahs',$mahs)->first();
            $modeldn = Company::where('maxa',$modelkk->maxa)
                ->first();
            $modelkkct = GiaVeTqKdlCt::where('mahs',$modelkk->mahs)
                ->get();
            ...
            return view('manage.kkgia.dvdlbb.reports.print')
                ...
                ->with('pageTitle','Kê khai giá vé tham quan khu du lịch');
    ...
    public function edit($id){
        if (Session::has('admin')) {
            if(session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X'){
                $delctdf = GiaVeTqKdlCt::where('trangthai','CXD')->delete();
                $model = GiaVeTqKdl::findOrFail($id);
                $modelct = GiaVeTqKdlCt::where('mahs',$model->mahs)->get();
                $modeldn = Company::where('maxa', $model->maxa)
                    ->first();

                return view('manage.kkgia.vetqkdl.kkgia.kkgiadv.edit')
                    ->with('modeldn', $modeldn)
                    ->with('modelct',$modelct)
                    ->with('model',$model)
                    ->with('pageTitle', 'Kê khai giá vé tham quan khu du lịch chỉnh sửa');
            }else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->update($inputs))
                    $modelctdf = GiaVeTqKdlCt::where('mahs',$inputs['mahs'])
                        ->update(['trangthai' => 'XD']);
                return redirect('kekhaigiavetqkdl?&masothue='.$model->maxa);
            } else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
    ...
    public function delete(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = GiaVeTqKdl::findOrFail($id);
                if($model->delete()){
                    $modelct = GiaVeTqKdlCt::where('mahs',$model->mahs)
                        ->delete();
                }
                return redirect('kekhaigiavetqkdl?&masothue='.$model->maxa);
            } else {
    ...
    public function kiemtra(Request $request){
        ...
        if(isset($inputs['id'])){
            $model = GiaVeTqKdl::where('id',$inputs['id'])
                ->first();
    ...
    public function chuyen(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H') {
                $inputs = $request->all();
                $model = GiaVeTqKdl::where('id',$inputs['idchuyen'])
                    ->first();
                $inputs['trangthai'] = 'CD';
                ...
                if($model->update($inputs)){
                    ...
                    $modeldv = Town::where('maxa',$model->mahuyen)
                        ->first();
                    $dmnghe = DmNgheKd::where('manghe','TQKDL')
                        ->where('manganh','TQKDL')
                        ->first();
