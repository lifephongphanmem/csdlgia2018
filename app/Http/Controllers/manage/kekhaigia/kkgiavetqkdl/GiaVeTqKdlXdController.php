...
class GiaVeTqKdlXdController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = GiaVeTqKdl::join('company','company.maxa','=','giavetqkdl.maxa')
                    ->where('giavetqkdl.mahuyen',$inputs['mahuyen'])
                    ->where('giavetqkdl.trangthai',$inputs['trangthai'])
                    ->select('giavetqkdl.*','company.tendn')
                    ->get();
                return view('manage.kkgia.vetqkdl.kkgia.xetduyet.index')
                    ->with('model', $model)
                    ->with('inputs',$inputs)
                    ->with('modeldv',$modeldv)
                    ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá vé tham quan tại khu du lịch');
            }else
                return view('errors.perm');
        }else
    ...
    public function ttdn(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = GiaVeTqKdl::where('id',$inputs['id'])
                ->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = GiaVeTqKdl::where('id',$inputs['idtralai'])->first();
                ...
                return redirect('xetduyetgiavetqkdl?&trangthai='.$inputs['trangthai'].'&maxa='.$model->mahuyen);
            }else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiavetqkdl?&trangthai=DD&maxa='.$model->mahuyen);
        }else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = GiaVeTqKdl::leftJoin('giavetqkdlct','giavetqkdlct.mahs','=','giavetqkdl.mahs')
                ->leftjoin('company','company.maxa','=','giavetqkdl.maxa')
                ->whereYear('giavetqkdl.ngayhieuluc',$inputs['nam'])
                ->select('giavetqkdlct.*','company.tendn','giavetqkdl.ngayhieuluc','giavetqkdl.maxa')
                ->where('giavetqkdl.trangthai','DD');
            if($inputs['tthhdv'] != '')
                $model = $model->where('giavetqkdlct.tthhdv','like','%'.$inputs['tthhdv'].'%');
            $model = $model->get();

            return view('manage.kkgia.vetqkdl.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá vé tham quan tại khu du lịch');
        }else
            return view('errors.notlogin');
    }
}
