...
class GiaOtoNkSxController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $ttql = District::where('mahuyen',$modeldmnghe->mahuyen)
                    ->first();
                return view('manage.kkgia.otonksx.kkgia.kkgiadv.ttdn')
                    ->with('model', $model)
                    ->with('modeldv',$modeldv)
                    ->with('inputs',$inputs)
                    ->with('ttql',$ttql)
                    ->with('pageTitle', 'Danh sách doanh nghiệp kê khai giá ôtô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi');
    ...
    public function index(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                $model = GiaOtoNkSx::where('maxa', $inputs['masothue'])
                ...
                if(isset($modeldn)) {
                    ...
                    return view('manage.kkgia.otonksx.kkgia.kkgiadv.index')
                        ...
                        ->with('modeldv', $modeldv)
                        ->with('inputs', $inputs)
                        ->with('pageTitle', 'Danh sách hồ sơ kê khai giá ôtô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi');
                }else
                    return view('errors.perm');
        ...
        if (Session::has('admin')) {
            ...
            if(isset($modeldn)) {
                ...
                $idlk = GiaOtoNkSx::where('maxa',$inputs['masothue'])
                    ...
                    ->max('id');
                if(isset($idlk)){
                    $modellk = GiaOtoNkSx::where('id',$idlk)
                        ->first();
                ...
                return view('manage.kkgia.otonksx.kkgia.kkgiadv.create')
                    ->with('modeldn', $modeldn)
                    ->with('modelct',$modelct)
                    ->with('modellk',$modellk)
                    ->with('inputs', $inputs)
                    ->with('pageTitle', 'Kê khai giá ôtô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi thêm mới');
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->create($inputs)){
                    $modelctdf = GiaOtoNkSxCt::where('mahs',$inputs['mahs'])
                        ->update(['trangthai' => 'XD']);
                }
                return redirect('kekhaigiaotonksx?&masothue='.$inputs['maxa']);
            } else {
                return view('errors.perm');
            }
        }else
    ...
    public function show(Request $request){
        if (Session::has('admin')) {
            ...
            $modeldn = Company::where('maxa',$modelkk->maxa)
                ->first();
            $modelkkct = GiaOtoNkSxCt::where('mahs',$modelkk->mahs)
                ->get();
            ...
            return view('manage.kkgia.otonksx.reports.print')
                ...
                ->with('pageTitle','Kê khai giá ô tô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi');
        ...
        if (Session::has('admin')) {
            if(session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X'){
                ...
                return view('manage.kkgia.otonksx.kkgia.kkgiadv.edit')
                    ...
                    ->with('modelct',$modelct)
                    ->with('model',$model)
                    ->with('pageTitle', 'Kê khai giá ôtô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi chỉnh sửa');
            }else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->update($inputs))
                    $modelctdf = GiaOtoNkSxCt::where('mahs',$inputs['mahs'])
                        ->update(['trangthai' => 'XD']);
                return redirect('kekhaigiaotonksx?&masothue='.$model->maxa);
            } else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
    ...
    public function kiemtra(Request $request){
        ...
        if(isset($inputs['id'])){
            $model = GiaOtoNkSx::where('id',$inputs['id'])
                ->first();
    ...
    public function chuyen(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H') {
                $inputs = $request->all();
                $model = GiaOtoNkSx::where('id',$inputs['idchuyen'])
                    ->first();
                $inputs['trangthai'] = 'CD';
