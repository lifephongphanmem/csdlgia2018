...
namespace App\Http\Controllers\manage\kekhaigia\kkgiaotonksx;
...
class GiaOtoNkSxXdController extends Controller
{
    public function index(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $modeldmnghe = DmNgheKd::where('manganh','OTO')
                    ->where('manghe','OTO')
                    ->first();
                ...
                $model = GiaOtoNkSx::join('company','company.maxa','=','giaotonksx.maxa')
                    ->where('giaotonksx.mahuyen',$inputs['mahuyen'])
                    ->where('giaotonksx.trangthai',$inputs['trangthai'])
                    ->select('giaotonksx.*','company.tendn')
                    ->get();
                return view('manage.kkgia.otonksx.kkgia.xetduyet.index')
                    ->with('model', $model)
                    ->with('inputs',$inputs)
                    ->with('modeldv',$modeldv)
                    ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá ô tô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi');
            }else
                return view('errors.perm');
        }else
    ...
    public function ttdn(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = GiaOtoNkSx::where('id',$inputs['id'])
                ->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = GiaOtoNkSx::where('id',$inputs['idtralai'])->first();
                if($model->update($inputs)){
                    $modeldn = Company::where('maxa', $model->maxa)
                        ->first();
                    $modeldv = Town::where('maxa',$model->mahuyen)
                        ->first();
                    $dmnghe = DmNgheKd::where('manghe','OTO')
                        ->where('manganh','OTO')
                        ->first();
                ...
                return redirect('xetduyetgiaotonksx?&trangthai='.$inputs['trangthai'].'&maxa='.$model->mahuyen);
            }else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
    ...
    public function getsohsnhan($mahuyen){
        $idmax = GiaOtoNkSx::where('trangthai', 'DD')
            ...
            ->max('id');
        if (isset($idmax)) {
            $model = GiaOtoNkSx::where('id',$idmax)
                ->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $id = $inputs['idnhanhs'];
            $model = GiaOtoNkSx::findOrFail($id);
            $inputs['trangthai'] = 'DD';
            ...
            return redirect('xetduyetgiaotonksx?&trangthai=DD&maxa='.$model->mahuyen);
        }else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = GiaOtoNkSx::leftJoin('giaotonksxct','giaotonksxct.mahs','=','giaotonksx.mahs')
                ->leftjoin('company','company.maxa','=','giaotonksx.maxa')
                ->whereYear('giaotonksx.ngayhieuluc',$inputs['nam'])
                ->select('giaotonksxct.*','company.tendn','giaotonksx.ngayhieuluc','giaotonksx.maxa')
                ->where('giaotonksx.trangthai','DD');
            if($inputs['tthhdv'] != '')
                $model = $model->where('giaotonksxct.tthhdv','like','%'.$inputs['tthhdv'].'%');
            $model = $model->get();

            return view('manage.kkgia.otonksx.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá ô tô nhập khẩu, sản xuất trong nước dưới 15 chỗ ngồi');
        }else
            return view('errors.notlogin');
    }
}
