        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                return view('manage.kkgia.etanol.kkgia.kkgiadv.ttdn')
                    ...
                    ->with('inputs',$inputs)
                    ->with('ttql',$ttql)
                    ->with('pageTitle', 'Danh sách doanh nghiệp kê khai giá Etanol nhiên liệu không biến tính, khí tự nhiên hóa lỏng(LNG); khí thiên nhiên nén (CNG)');
            }else{
                return view('errors.perm');
            }
        }else
    ...
    public function index(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                $model = KkGiaEtanol::where('maxa', $inputs['masothue'])
                ...
                if(isset($modeldn)) {
                    ...
                    return view('manage.kkgia.etanol.kkgia.kkgiadv.index')
                        ...
                        ->with('modeldv', $modeldv)
                        ->with('inputs', $inputs)
                        ->with('pageTitle', 'Danh sách hồ sơ kê khai Etanol nhiên liệu không biến tính, khí tự nhiên hóa lỏng(LNG); khí thiên nhiên nén (CNG)');
                } else
                    return view('errors.perm');
    ...
    public function create(Request $request){
        if (Session::has('admin')) {
            ...
            $modeldn = Company::join('companylvcc','companylvcc.maxa','=','company.maxa')
                ...
                ->first();
            if(isset($modeldn)) {
                $delctdf = KkGiaEtanolCtDF::where('maxa',$inputs['masothue'])->delete();
                $idlk = KkGiaEtanol::where('maxa',$inputs['masothue'])
                    ->where('trangthai','DD')
                    ->max('id');
                if(isset($idlk)){
                    $modellk = KkGiaEtanol::where('id',$idlk)
                        ->first();
                    $modellkct = KkGiaEtanolCt::where('mahs',$modellk->mahs)
                        ->get();
                    foreach($modellkct as  $ctdf){
                        $addct = new KkGiaEtanolCtDF();
                ...
                $modelct = KkGiaEtanolCtDF::where('maxa',$inputs['masothue'])
                    ->get();
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->create($inputs)){
                    $modelctdf = KkGiaEtanolCtDF::where('maxa',$inputs['maxa']);

                    foreach($modelctdf->geT() as $ctdf) {
                        $modelct = new KkGiaEtanolCt();
                        $arrays = $ctdf->toArray();
                        unset($arrays['id']);
                ...
                return redirect('kekhaigiaetanol?&masothue='.$inputs['maxa']);
            } else {
                return view('errors.perm');
            }
        }else
    ...
    public function show(Request $request){
        if (Session::has('admin')) {
            ...
            $modeldn = Company::where('maxa',$modelkk->maxa)
                ->first();
            $modelkkct = KkGiaEtanolCt::where('mahs',$modelkk->mahs)
                ->get();
            ...
            return view('manage.kkgia.etanol.reports.print')
                ...
                ->with('pageTitle','Kê khai giá Etanol nhiên liệu không biến tính, khí tự nhiên hóa lỏng(LNG); khí thiên nhiên nén (CNG)');
        ...
        if (Session::has('admin')) {
            if(session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X'){
                ...
                return view('manage.kkgia.etanol.kkgia.kkgiadv.edit')
                    ...
                    ->with('modelct',$modelct)
                    ->with('model',$model)
                    ->with('pageTitle', 'Kê khai giá Etanol nhiên liệu không biến tính, khí tự nhiên hóa lỏng(LNG); khí thiên nhiên nén (CNG) chỉnh sửa');
            }else
                return view('errors.perm');
        }else
            return view('errors.notlogin');
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($inputs['ngaycvlk'] != '')
                    ...
                    unset($inputs['ngaycvlk']);
                $model->update($inputs);
                return redirect('kekhaigiaetanol?&masothue='.$model->maxa);
            } else {
                return view('errors.perm');
            }
        }else
    ...
    public function chuyen(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H') {
                $inputs = $request->all();
                $model = KkGiaEtanol::where('id',$inputs['idchuyen'])
                    ->first();
                $inputs['trangthai'] = 'CD';
                ...
                if($model->update($inputs)){
                    ...
                    $modeldv = Town::where('maxa',$model->mahuyen)
                        ->first();
                    $dmnghe = DmNgheKd::where('manghe','ETANOL')
                        ->where('manganh','ETANOL')
                        ->first();
