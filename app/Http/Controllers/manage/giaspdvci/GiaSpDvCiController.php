...
class GiaSpDvCiController extends Controller
{
    ...
    public function congbo(Request $request){
        if(Session::has('admin')){
            ...
            $model->trangthai = 'CB';
            $model->save();
            $nam = date('Y',strtotime($model->ngayqd));
            return redirect('giaspdvci?&nam='.$nam);
        }else
            return view('errors.notlogin');
    }
    ...
    public function show($id){
        if(Session::has('admin')) {
            ...
            return view('manage.dinhgia.giaspdvci.show')
                ->with('model', $model)
                ->with('modelct',$modelct)
                ->with('inputs',$inputs)
                ->with('pageTitle', 'Giá sản phẩm dịch vụ công ích chi tiết');
        }else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if(Session::has('admin')){
            ...
            $model = GiaSpDvCiCt::join('giaspdvci','giaspdvci.mahs','=','giaspdvcict.mahs')
                ->select('giaspdvci.ngayqd','giaspdvci.ttqd','giaspdvci.soqd','giaspdvcict.*');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('ngayqd',$inputs['nam']);
            if($inputs['mota'] != '')
                $model = $model->where('giaspdvcict.mota','like', '%'.$inputs['mota'].'%');
            $model = $model->paginate($inputs['paginate']);

            return view('manage.dinhgia.giaspdvci.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin giá sản phẩm, dịch vụ công ích');
        }else
            return view('errors.notlogin');
    }
}
