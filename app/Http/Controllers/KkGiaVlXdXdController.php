...
class KkGiaVlXdXdController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = KkGiaVlXd::join('company','company.maxa','=','kkgiavlxd.maxa')
                    ...
                    ->whereYear('kkgiavlxd.ngaychuyen',$inputs['nam']);
    ...
    public function ttdnkkvlxd(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaVlXd::where('id',$inputs['id'])
                ->first();
            $modeldn = Company::where('maxa',$modelhs->maxa)->where('level','VLXD')->first();
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                if($model->update($inputs)){
                    ...
                    $dn = Company::where('maxa',$model->maxa)->where('level','VLXD')->first();
                    $data=[];
                    $data['tendn'] = $dn->tendn;
    ...
    public function getsohsnhan($mahuyen){
        $model = KkGiaVlXd::where('trangthai', 'DD')
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGiaVlXdCt::leftJoin('kkgiavlxd','kkgiavlxd.mahs','=','kkgiavlxdct.mahs')
                ...
                ->wherein('kkgiavlxd.trangthai',['DD','CB']);
            if($inputs['ten'] != '')
