...
class KkGiaKcbTnXdController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = KkGiaKcbTn::join('company','company.maxa','=','kkgiakcbtn.maxa')
                    ...
                    ->where('company.level','KCBTN')
                    ->where('kkgiakcbtn.trangthai',$inputs['trangthai'])
                    ->whereYear('kkgiakcbtn.ngaychuyen',$inputs['nam']);
                ...
                $model = $model->where('kkgiakcbtn.mahuyen',$inputs['maxa']);

                $model = $model->get();
                return view('manage.kkgia.kcbtn.kkgia.xetduyet.index')
                    ->with('model', $model)
                    ->with('inputs',$inputs)
                    ->with('modeldvql',$modeldvql)
                    ->with('modeldv',$modeldv)
                    ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá Dịch vụ khám chữa bệnh cho người tại cơ sở khám chữa bệnh tư nhân; khám chữa bệnh theo yêu cầu tại cơ sở khám chữa bệnh của nhà nước');
            }else{
    ...
    public function ttdn(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaKcbTn::where('id',$inputs['id'])
                ->first();
            $modeldn = Company::where('maxa',$modelhs->maxa)->where('level','KCBTN')->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                $model = KkGiaKcbTn::where('id',$inputs['idtralai'])->first();
                ...
                return redirect('xetduyetgiakcbtn?&trangthai='.$inputs['trangthai'].'&maxa='.$model->mahuyen);
            }else{
                return view('errors.perm');
            }
    ...
    public function getsohsnhan($mahuyen){
        $model = KkGiaKcbTn::where('trangthai', 'DD')
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiakcbtn?&trangthai=DD&maxa='.$model->mahuyen);
        }else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGiaKcbTn::leftJoin('kkgiakcbtnct','kkgiakcbtnct.mahs','=','kkgiakcbtn.mahs')
                ->leftjoin('company','company.maxa','=','kkgiakcbtn.maxa')
                ->where('company.level','KCBTN')
                ->whereYear('kkgiakcbtn.ngayhieuluc',$inputs['nam'])
                ->select('kkgiakcbtnct.*','company.tendn','kkgiakcbtn.ngayhieuluc','kkgiakcbtn.maxa')
                ->where('kkgiakcbtn.trangthai','DD');
            if($inputs['tthhdv'] != '')
                $model = $model->where('kkgiakcbtnct.tthhdv','like','%'.$inputs['tthhdv'].'%');
            $model = $model->get();

            return view('manage.kkgia.kcbtn.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá Dịch vụ khám chữa bệnh cho người tại cơ sở khám chữa bệnh tư nhân; khám chữa bệnh theo yêu cầu tại cơ sở khám chữa bệnh của nhà nước');
        }else
            return view('errors.notlogin');
    }
}
