...
<table cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;">
    <tr>
        ...
        <th @if($t1 ==0) style='display: none' @endif colspan="{{$t1}}">Tháng 1/{{$nam}}</th>
        <th @if($t2 ==0) style='display: none' @endif colspan="{{$t2}}">Tháng 2/{{$nam}}</th>
        <th @if($t3 ==0) style='display: none' @endif colspan="{{$t3}}">Tháng 3/{{$nam}}</th>
        <th @if($t4 ==0) style='display: none' @endif colspan="{{$t4}}">Tháng 4/{{$nam}}</th>
        <th @if($t5 ==0) style='display: none' @endif colspan="{{$t5}}">Tháng 5/{{$nam}}</th>
        <th @if($t6 ==0) style='display: none' @endif colspan="{{$t6}}">Tháng 6/{{$nam}}</th>
        <th @if($t7 ==0) style='display: none' @endif colspan="{{$t7}}">Tháng 7/{{$nam}}</th>
        <th @if($t8 ==0) style='display: none' @endif colspan="{{$t8}}">Tháng 8/{{$nam}}</th>
        <th @if($t9 ==0) style='display: none' @endif colspan="{{$t9}}">Tháng 9/{{$nam}}</th>
        <th @if($t10 ==0) style='display: none' @endif colspan="{{$t10}}">Tháng 10/{{$nam}}</th>
        <th @if($t11 ==0) style='display: none' @endif colspan="{{$t11}}">Tháng 11/{{$nam}}</th>
        <th @if($t12 ==0) style='display: none' @endif colspan="{{$t12}}">Tháng 12/{{$nam}}</th>
    </tr>
    <tr>
    ...
    @foreach($donvi as $dv)
        <tr style="font-weight: bold; font-style: italic">
            ...
            <td style="text-align: left;" >{{$dv->tendn}}</td>
            @foreach($a_t1 as $key=>$val)
                <th></th>
            @endforeach
            @foreach($a_t2 as $key=>$val)
