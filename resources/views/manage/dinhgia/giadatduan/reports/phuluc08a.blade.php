...
<p style="font-weight: bold;text-transform: uppercase;text-align: center">THÔNG TIN VỀ GIÁ ĐẤT CỤ THỂ CỦA DỰ ÁN TRÊN ĐỊA BÀN {{$modeldb->diaban}} - Năm {{$inputs['nam']}}</p>
<table cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;" id="data">
    ...
    @foreach($modeldm as $dm)
        <tr>
            ...
            <?php
                $modeltt = $model->where('manhomduan',$dm->manhomduan)
                    ->where('mahuyen',$modeldb->district);
            ?>
            @if(count($modeltt)>0)
                <tr>
                    <td style="text-align: center;font-weight: bold;">{{IntToRoman(1)}}</td>
                    <td colspan="20" style="font-weight: bold;">{{$modeldb->diaban}}</td>
                </tr>
