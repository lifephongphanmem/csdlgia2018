...
@section('custom-script')
    ...
    <script>
        ...
        function searchtt(){
            var current_path_url = '/giathuenhacongvu?';
            var nam = '&nam='+$('#nam').val();
        ...
        function resettt(){
            window.location.href = '/giathuenhacongvu';
        }

        function edittt(id) {
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: 'giathuenhacongvu/edittt',
                type: 'GET',
                data: {
                    _token: CSRF_TOKEN,
                    id: id
...
@section('content')
    ...
    <h3 class="page-title">
        Giá thuê <small>&nbsp;nhà công vụ</small>
    </h3>
    <!-- END PAGE HEADER-->
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="portlet-title">
                    <div class="actions">
                        ...
                        <button type="button" class="btn btn-default btn-xs mbs" data-target="#delete-modal-confirm" data-toggle="modal"><i class="fa fa-trash-o"></i>&nbsp;
                            Xóa</button>
                        @endif
                        @if(can('thgiathuenhacongvu','congbo'))
                        <button type="button" class="btn btn-default btn-xs mbs" data-target="#check-modal-confirm" data-toggle="modal"><i class="fa fa-check"></i>&nbsp;
                            Công bố/ Hủy</button>
                        @endif
                        <a href="{{url('giathuenhacongvu/prints?&nam='.$inputs['nam'].'&district='.$inputs['district'].'&tenduan='.$inputs['tenduan'])}}" class="btn btn-default btn-sm" target="_blank">
                                    ...
                                    @foreach($model as $key => $tt)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @elseif($tt->trangthai == 'CHT')
                                                    {{--Chưa hoàn thành--}}
                                                    @if(can('kkgiathuenhacongvu','edit'))
                                                    ...
                                                    @endif
                                                    @if(can('kkgiathuenhacongvu','approve'))
    ...
    @include('manage.dinhgia.thuenhacongvu.include.modal_dialog')
