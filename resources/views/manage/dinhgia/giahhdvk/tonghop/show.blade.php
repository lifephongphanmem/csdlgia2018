...
@section('content')
    ...
    <p style="text-align: center; font-weight: bold; font-size: 16px;text-transform: uppercase;">BẢNG GIÁ THỊ TRƯỜNG  THÁNG {{$model->thang}} NĂM {{$model->nam}}</p>
            ...
            @foreach($ttct as $key=>$tt)
                <tr>
                    ...
                    <td style="text-align: right;font-weight: bold">{{number_format($tt->gialk) == 0 ? ''
                                : dinhdangsothapphan(($tt->gia-$tt->gialk)/$tt->gialk,5)}}</td>
                    <td>{{$tt->nguontt}}</td>
                    <td>{{$tt->ghichu}}</td>
                </tr>
            @endforeach
