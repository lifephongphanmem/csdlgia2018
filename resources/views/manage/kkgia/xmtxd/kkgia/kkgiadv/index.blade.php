    ...
    <script>
        ...
        $(function(){
            ...
            $('#namhs').change(function() {
                ...
                var url = '/thongtinkekhaiximangthepxaydung?&masothue='+masothue+'&nam='+nam+'&trangthai='+trangthai;
...
@section('content')
    ...
    <h3 class="page-title">
        Thông tin kê khai giá<small>&nbsp;xi măng, thép xây dựng</small>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="actions">
                        @if(can('kkxmtxd','create'))
                                <a href="{{url('thongtinkekhaiximangthepxaydung/create?&masothue='.$inputs['masothue'])}}" class="btn btn-default btn-sm">
                                    <i class="fa fa-plus"></i> Kê khai mới </a>
                        @endif
                        @if(session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X')
                            <a href="{{url('thongtindnkkgiaxmtxd?&maxa='.$modeldn->mahuyen)}}" class="btn btn-default btn-sm">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_3">
                        ...
                        @foreach($model as $key=>$tt)
                            <tr>
                                ...
                                @endif
                                <td>
                                    <a href="{{url('thongtinkekhaiximangthepxaydung/prints?&mahs='.$tt->mahs)}}" target="_blank" class="btn btn-default btn-xs mbs"><i class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                    @if(canEdit($tt->trangthai))
                                        @if(can('kkvlxd','edit'))
                                            <a href="{{url('thongtinkekhaiximangthepxaydung/'.$tt->id.'/edit')}}" class="btn btn-default btn-xs mbs"><i class="fa fa-edit"></i>&nbsp;Chỉnh sửa</a>
                                        @endif
                                        @if(canChuyenXoa($tt->trangthai))
                                            @if(can('kkvlxd','delete'))
                                                @if($tt->trangthai == 'CC')
                                            <button type="button" onclick="getId('{{$tt->id}}')" class="btn btn-default btn-xs mbs" data-target="#delete-modal" data-toggle="modal"><i class="fa fa-trash-o"></i>&nbsp;
                                                Xóa</button>
                                                @endif
                                            @endif
                                            @if(can('kkvlxd','approve'))
        ...
        <div class="modal fade" id="chuyen-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'thongtinkekhaiximangthepxaydung/chuyen','id' => 'frm_chuyen'])!!}
    ...
    <!--Model chuyển hs chậm-->
        <div class="modal fade" id="chuyenhscham-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'thongtinkekhaiximangthepxaydung/chuyenhscham','id' => 'frm_chuyenhscham'])!!}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
    ...
    <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                {!! Form::open(['url'=>'thongtinkekhaiximangthepxaydung/delete','id' => 'frm_delete'])!!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
