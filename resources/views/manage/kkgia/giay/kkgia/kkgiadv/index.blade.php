    ...
    <script>
        ...
        $(function(){
            $('#namhs').change(function() {
                ...
                var url = '/kekhaigiagiay?&masothue='+masothue+'&nam='+nam;
...
@section('content')
    <marquee>
        <b style="color: #ff0000">{{$modeldv->tendv}} xin thông báo:</b> Ngày áp dụng hồ sơ kê khai giá giấy in, viết (dạng cuộn), giấy in báo sản xuất trong nước phải sau {{$modeldv->songaylv}} ngày làm việc, tính từ thời điểm chuyển hồ sơ. Hồ sơ chuyển  trước 17h sẽ tính từ ngày gửi, sau 17h sẽ tính ngày hôm sau!!! (Ngày làm việc không tính thứ 7, CN và ngày nghỉ lể)
    </marquee>
    <h3 class="page-title">
        Thông tin kê khai giá<small>&nbsp;giấy in, viết (dạng cuộn), giấy in báo sản xuất trong nước</small>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="actions">
                        <a href="{{url('kekhaigiagiay/create?&masothue='.$inputs['masothue'])}}" class="btn btn-default btn-sm">
                            <i class="fa fa-plus"></i> Kê khai mới </a>
                        @if(isset($inputs['hsdd']) && $inputs['hsdd'] == 0)
                            <a href="{{url('kekhaigiagiay/nhandulieutuexcel?&maxa='.$inputs['masothue'])}}" class="btn btn-default btn-sm">
                                <i class="fa fa-file-excel-o"></i> Nhận dữ liệu</a>
                        @endif
                        @if(session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X')
                            <a href="{{url('thongtindngiay?&maxa='.$modeldn->mahuyen)}}" class="btn btn-default btn-sm">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_3">
                        ...
                        @foreach($model as $key=>$tt)
                            <tr>
                                ...
                                @endif
                                <td>
                                    <a href="{{url('kekhaigiagiay/prints?&mahs='.$tt->mahs)}}" target="_blank" class="btn btn-default btn-xs mbs"><i class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                    @if(canEdit($tt->trangthai))
                                        @if(can('kkgiay','edit'))
                                            <a href="{{url('kekhaigiagiay/'.$tt->id.'/edit')}}" class="btn btn-default btn-xs mbs"><i class="fa fa-edit"></i>&nbsp;Chỉnh sửa</a>
                                        @endif
        ...
        <div class="modal fade" id="chuyen-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'kekhaigiagiay/chuyen','id' => 'frm_chuyen'])!!}
    ...
    <!--Model chuyển hs chậm-->
        <div class="modal fade" id="chuyenhscham-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'kekhaigiagiay/chuyenhscham','id' => 'frm_chuyenhscham'])!!}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
    ...
    <!--Model copy-->
        <div class="modal fade" id="copy-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'kekhaigiagiay/copy','id' => 'frm_chuyen'])!!}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
    ...
    <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                {!! Form::open(['url'=>'kekhaigiagiay/delete','id' => 'frm_delete'])!!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
